...
Welcome to your first off platform project using RStudio! There are a few goals of this project. The first is to get practice using some of the basic R commands you have learned so far. However, the second goal
...
temps_f <- c(62, 59, 55, 48, 55, 57, 59)
```
...
f_to_c <- function(fahrenheit, celsius) {
...
temps_cities <- list(
  Toulon = c(62, 59, 55, 48, 55, 57, 59),
  Marseille = c(56, 58, 55, 51, 55, 57, 58),
  Nice = c(65, 58, 54, 53, 55, 57, 60)
)
temps_cities
```
...
is_nice_weather(temps_cities$Toulon)
is_nice_weather(temps_cities$Marseille)
is_nice_weather(temps_cities$Nice)
```
...
library(ggplot2)
destination_df <- as.data.frame(temps_cities)
